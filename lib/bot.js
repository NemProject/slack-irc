...
Nem.prototype.balance10 = function(address, cb) {
  return this.balanceGen(address, 4, cb);
}
...
Bot.prototype.sendToChan = function(ircChannel, chan, textData) {
  ...
  var message = {
    ...
    username: 'tipbot',
    parse: 'full',
    icon_url: 'http://nem.io/img/64x64.png'
  };
...
Bot.prototype.sendWithdraw = function(user, body) {
  ...
  if (destAddr == address) {
    self.sendToUser(user, 'hey man, what are you trying to achive by this?');
    return;
  }
...
Bot.prototype.handleCommands = function(dest, body) {
	...
	case "tip":
      ...
      var p = body.match(/.*(send|!tip|tip|!deposit)/i);
      if (p) {
        if (p[1] === '!deposit') {
          this.createDeposit(user, function(deposit, code) {
            self.sendToChan(ircChannel, chan, 'User @' + user.name + ', your deposit account is ' + deposit + '\n');
          });
        } else {
          this.sendSend(dest, body);
        }
